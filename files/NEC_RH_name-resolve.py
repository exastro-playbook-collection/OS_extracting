...
if os.path.isfile(filename1):
    ...
    if name_server_list ==[] and suff_list !=[]:
        result['VAR_NEC_name_resolve_dns']={'suffix':suff_list}
    if name_server_list !=[] and suff_list ==[]:
        result['VAR_NEC_name_resolve_dns']={'server':name_server_list}
    if name_server_list !=[] and suff_list !=[]:
        result['VAR_NEC_name_resolve_dns']={'server':name_server_list,'suffix':suff_list }
...
for res in key_table1:
    result.setdefault('VAR_NEC_name_resolve_hosts',[]).append(res)

result.setdefault('VAR_NEC_RH_name_resolve_ipv6_disabeld',False)
